...
class ConnectDrawer:
    ...
    def draw_one_connection(self, connection_def:tuple) -> None: 
        ...
        if inp_id + 1 < num_inputs/2: 
            ...
            d_coord = (d_coord[0] - self.padding * (num_inputs%inp_id), d_coord - self.padding *(num_inputs%inp_id))
        else: 
        ...
        curr_coord = d_coord
        nav_tup = self.navigate_intersection(curr_coord, (end_x, end_y), self.domain_dict, inp_obj, out_obj)
        ...
        while nav_tup[0]: 
            ...
            if abs(curr_coord[0] - min_x) < abs(curr_coord[0] - max_x):
                next_x_coord = min_x - self.padding* inp_id
                next_y_coord = curr_coord[1]
            else: 
                next_x_coord = max_x + self.padding * inp_id
                next_y_coord = curr_coord[1]
    ...
    def navigate_intersection(self, curr_coord, dest_coord, domain_dict, inp_obj, out_obj): 
        ...
        for key in domain_dict.keys(): 
            ...
            if key is inp_obj or key is out_obj: 
                continue
