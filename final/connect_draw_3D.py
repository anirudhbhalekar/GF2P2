"""
This module defines the ConnectDrawer3D class, which handles 3D drawing
of connections using OpenGL in a wxPython application.
"""

import numpy as np
from OpenGL.GL import (
    GL_LINE_STRIP, GL_LINE_LOOP, GL_POLYGON, GL_ENABLE_BIT, GL_LINE_STIPPLE,
    glBegin, glEnd, glVertex2f, glColor3f, glPushAttrib, glLineStipple, glPopAttrib, glEnable
)
from OpenGL import GL, GLUT


class ConnectDrawer3D:
    """Class for drawing 3D connections using OpenGL."""
    ...
    def return_tube_vertices(self, curr_coords, dest_coords) -> list:
        ...
        if index == 0:
            ...
            dx = -1 if direction_vector[0] < 0 else 1
            ...
            dy = -1 if direction_vector[1] < 0 else 1
            ...
            dz = -1 if direction_vector[2] < 0 else 1
        ...
        f1, f2 = [0, 1, 2, 3], [4, 5, 6, 7]
        f3, f4 = [1, 5, 6, 2], [0, 3, 7, 4]
        f5, f6 = [2, 6, 7, 3], [0, 4, 5, 1]
        ...
        for i, face in enumerate(face_list):
            ...
            for index in face:
                ...
                vertices.extend([vertex[0], vertex[1], vertex[2], 0.5, 0.5, face_normal[0], face_normal[1], face_normal[2]])

        return list(vertices)

    def make_all_connections(self) -> list:
        """
        Returns a list of vertices for all connections in the form 
        (x, y, z, tex_coord, tex_coord, n_x, n_y, n_z).
        * tex_coord is irrelevant.
        """
        vertices = []
        count = 0
        for device in self.devices.devices_list:
            ...
            for input_id in device.inputs.keys():
                ...
                is_top = input_id == self.devices.SET_ID
                is_bot = input_id == self.devices.CLEAR_ID
                ...
                if output_coords is not None:
                    this_vertices = self.make_single_connection(
    ...
    def make_single_connection(
        self, input_coords, output_coords, z_offset,
        is_input_top: bool = False, is_input_bot: bool = False
    ) -> list:
        """
        Makes a single connection in 6 stages:
    ...
    def draw_connections(self, vertices: list) -> None:
        self.deprecated_draw(vertices)

    def deprecated_draw(self, vertices: list) -> None:
        """Draws the connections using deprecated OpenGL methods."""
        quad_vertices = []
        quad_normals = []
        this_vertex = []
        this_normal = []
        ...
        for index, element in enumerate(vertices):
            ...
            if len(quad_normals) == 4:
                ...
                GL.glNormal3f(*quad_normals[0])
                GL.glVertex3f(*quad_vertices[0])
                GL.glVertex3f(*quad_vertices[1])
                GL.glVertex3f(*quad_vertices[2])
                GL.glVertex3f(*quad_vertices[3])
                GL.glEnd()
                quad_vertices.clear()
                quad_normals.clear()
